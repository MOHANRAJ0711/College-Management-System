...
/**
 * GET /api/faculty — admin can see all; faculty (HOD) sees their department only
 */
const getFaculties = async (req, res) => {
  try {
    const query = { ...req.query };

    // If faculty role is fetching, restrict to their own department (HOD access)
    if (req.user.role === 'faculty') {
      const ownFaculty = await Faculty.findOne({ user: req.user._id });
      if (ownFaculty && ownFaculty.department) {
        query.department = ownFaculty.department.toString();
      } else {
        // Faculty without a department cannot list others
