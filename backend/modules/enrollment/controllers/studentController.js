...
/**
 * Build MongoDB filter for Student queries.
 * @param {object} query  — Request query params
 * @param {object} user   — Authenticated user (req.user); used to scope faculty/HOD access
 */
const buildStudentFilter = async (query, user) => {
  ...
  // HOD faculty: restrict to their own department
...
const getStudentDashboard = async (req, res) => {
  try {
    ...
    // Count 'present' AND 'late' as attended (late means still came to class)
    const attended = attendanceRecords.filter(
      (r) => r.status === 'present' || r.status === 'late'
    ).length;
    const attendancePercent = total === 0 ? null : Math.round((attended / total) * 1000) / 10;
